...
@Config
@TeleOp(name="CameraCalibration", group="Testing")
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            if (!tagProcessor.getDetections().isEmpty()) {
                ...
                positionX = tag.ftcPose.range * Math.cos(Math.toRadians(theta)) + 11.3;
                positionY = tag.ftcPose.range * Math.sin(Math.toRadians(theta)) + 11.25;

                if (currentGamepad1.options && !previousGamepad1.options) {
                    new Pose2d(positionX,positionY,Math.toRadians(theta));
